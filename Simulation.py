...
for i in range(0, len(SimVar.sellingPoliciesLong)):
    ...
    while Day <= SimVar.SimRunTime:
        ...
        if (Day % SimVar.sellingPoliciesLong[i][0] == 1) and Day >= BeginSellingDate:
            PigletDF, PigletPandasDF, new_earnings = return_pig_value(Day, PigletDF, SimVar.sellingPoliciesLong[i][1])
            totalEarnings += new_earnings
            SoldPigsDF = pd.concat([SoldPigsDF, PigletPandasDF])
        Day += 1

    # extract piglet and slaughter data
    SoldPigsDF.to_csv("SoldPigsDF_logistic_model_1_policy_" + str(SimVar.sellingPoliciesLong[i]) + ".csv")
    print("Total earnings: {}".format([str(SimVar.sellingPoliciesLong[i]), totalEarnings]))
    totalEarningsList.append(totalEarnings)
    SoldPigsList.append(len(SoldPigsDF))
    sellingPoliciesList.append(str(SimVar.sellingPoliciesLong[i]))
